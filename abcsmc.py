...
class FittingData:
    ...
    def AddSimulatedData(self,newSimData):
        """
        Append new data to the existing data attribute simulatedData
        """
...
class ModelFitting:
    ...
    def RejectionSampling(self,ODESolveFunc,fSampleFunc,fAccept,paramList=None,fScore=None,popSize=None,rejectionTime=None,fileObj=None,CheckConstraintFunc=None): 
        ...
        # If the time limit for rejection sampling is not set, then the default time limit is half an hour
        ...
        # initialize the number of parameters and the associated data structures related to the optimization algorithm 
        ...
        count = 0
        fileObj.write('\r')
        # the flag to determine whether the optimization is successfully terminated or not
        success  = True 
        while (count < popSize) and success == True:
            # Use fPriorSample to generate a single sample of parameter vector ( in the log range of reference vector ) from the prior distribution 
            priorP = fSampleFunc()
			# Use priorP to calcualte the actual values of the parameters to be put into the 
            K = self.GetFittingData().GetReferenceVec()*10**(priorP)
            # Simulate the model with the given parameters. The output is a dictionary of simulated time course of state variables and associated list of state variable names 
            outputODESolver = ODESolveFunc(K)
            # check for the constraints 
            outputCheck = True
            if CheckConstraintFunc:
				# outputCheck is a boolean variable that equals True if the optimization is successfully terminated. Falase otherwise 
                outputCheck = CheckConstraintFunc(outputODESolver)
            # score the simulated data against the real data         
            if outputCheck: 
            ...
            # update the number of attempts of sampling 
			totalAttempts += 1
			# Check if the run time of the rejection has reached the limit 
            if rejectionTime < (datetime.now() -t0).total_seconds()/3600:
                success = False
	    # Calculate the final running time of the rejection sampling  
    ...
    def SequentialMonteCarlo(self,**kwargs):
        ...
        # The schedule of alpha. The potential particle has to score within the top alpha fraction of the previous iteration 
        ...
        # Time limit of rejection sampling 
        ...
        if success == True: 
            # Iteratively sample for the posteriors
            while (~canTerminate) and success == True:
                ...
                # The option to use schedule of epsilon
				if option == 1:
                ...
                # The option to use schedule of alpha
				elif option == 2:
